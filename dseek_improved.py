...
# Handle numeric columns (only if they exist in your dataset)
for col in ['RETAIL SALES', 'WAREHOUSE SALES', 'RETAIL TRANSFERS']:
    if col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

# Final cleanup
df = df.dropna(subset=['DATE', 'RETAIL SALES'])  # Keep rows with valid DATE and RETAIL SALES
...
# Optional custom index string (for advanced meta/SEO)
app.index_string = '''
<!DOCTYPE html>
<html>
    <head>
...
app.layout = dbc.Container(
    ...
    children=[
        ...
        dbc.Row([
            ...
            dbc.Col(
                # Show only if ITEM TYPE column exists
                dcc.Dropdown(
                    id="item_type_filter",
...
def update_charts(selected_year, selected_supplier, selected_item_type, start_date, end_date):
    try:
        ...
        # Only show breakdown when "All" is selected; otherwise, notify the user.
        ...
        return trend_fig, heatmap_fig, suppliers_fig, item_type_fig
